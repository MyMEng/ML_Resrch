...
\begin{enumerate}
\item \texttt{Maximum exploitation}--- during first boosting iteration all $4/4$ and half of $3/4$ instances are added. Next steps are only taken if there are any elements in mentioned groups and are added based on rule: append elements with higher probability first,
\item \texttt{Sustainable exploitation}--- during all boosting rounds append only elements from $4/4$ group,
\item \texttt{Data exploration}--- during first boosting iteration all $4/4$ are appended. All the next steps consists of adding ,
\item \texttt{No exploration}--- append all instances during first boosting round.
\end{enumerate}
Above experiments are conducted with $0.25\%\text{, } 1\%\text{, } 5\%\text{, } 10\% \text{ and } 25\%$ of training set volume.
...
\section{Results}
This section will be divided into subsections each corresponding to percentage of initial training size. Further discussion comparing each of data sets and taken approach will be included in order depending on outcomes of particular experiment.

\subsection{$\mathbf{25\%}$ sample size}
We will first discuss results for initial sample of size $25\%$ of original training set.

\subsection{$\mathbf{10\%}$ sample size}

\subsection{$\mathbf{5\%}$ sample size}

\subsection{$\mathbf{1\%}$ sample size}

\subsection{$\mathbf{0.25\%}$ sample size}
