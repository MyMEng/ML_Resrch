...
\usepackage[top=0.75in, bottom=1.44in, left=0.75in, right=0.75in]{geometry}
\usepackage{amsmath}
...
While running the experiment it is possible to decide how many instances from the above categories should be used for boosting procedure. Selected instances are labeled based on above rules and appended to training set. With classifiers re-trained on expanded set remaining unlabeled instances are are classified and grouped again. This process is repeated until person running the experiment decides to stop boosting procedure.\\
...
To get more details about input and output of the program please follow to \textbf{Appendix~\ref{ap:code}} or find \texttt{README.md} in GitHub repository\cite{githubcode}.

\section{The experiment}
To make tests more diverse we decided to use one dataset from each category:
\begin{itemize}
\item single binary target label,
\item single multivalued target label,% beyond binary classification,
...
\begin{itemize}
\item \texttt{mushrooms}--- abc,
\item \texttt{mushrooms}--- 123,
\item \texttt{mushrooms}--- IIIIII.
\end{itemize}

\subsection{Approaches to the experiment}


% include morkdown later on
