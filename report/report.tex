...
\item \texttt{Maximum exploration}--- during first round append all instances from $4/4$ group. Then, in each round add maximum of $10\%$ of initial number of instances in $U$ with priority given to groups with higher rank. Repeat the last step until no instances are left.
\end{enumerate}
Above experiments are conducted with $0.25\%\text{, } 5\% \text{ and } 25\%$ of whole training set ($U$) volume.
...
\subsection{$\mathbf{5\%}$ sample~\cite{results5}}
\lipsum[1]
